...
DBCricleHandler::DBCricleHandler(void)
{
}
...
void DBCricleHandler::handle(boost::asio::io_service &s, hgSqlOperator& sqlOperator)
{
    switch (Type)
    {
...
void DBCricleHandler::Addhandle(boost::asio::io_service &s, hgSqlOperator& sqlOperator)
{
    hgSqlInsertCmd* l_pSqlInsertCmd = new hgSqlInsertCmd;
    l_pSqlInsertCmd->SetTableName("cricle_table");
    ...
    {
		std::cout << "Open datatabase error(cricle_table add):" << sqlOperator.LastError().text().toLatin1().data() << endl;
...
void DBCricleHandler::Updatehandle(boost::asio::io_service &s, hgSqlOperator& sqlOperator)
{
    hgSqlUpdateCmd* l_pSqlUpdateCmd = new hgSqlUpdateCmd;
    l_pSqlUpdateCmd->SetTableName("cricle_table");
    QMap<QString, QVariant> l_data;
    //l_data.insert("ID",ID);
    l_data.insert("name",Name);
    l_data.insert("lat",Lat);
    ...
    {
		std::cout << "Open datatabase error(cricle_table Update):" << sqlOperator.LastError().text().toLatin1().data() << endl;
...
void DBCricleHandler::Removehandle(boost::asio::io_service &s, hgSqlOperator& sqlOperator)
{
    hgSqlRemoveCmd* l_pSqlRemoveCmd = new hgSqlRemoveCmd;
    l_pSqlRemoveCmd->SetTableName("cricle_table");
    ...
    {
		std::cout << "Open datatabase error(cricle_table Remove):" << sqlOperator.LastError().text().toLatin1().data() << endl;
...
// void DBCricleHandler::Allhandle(boost::asio::io_service &s, hgSqlOperator& sqlOperator)
// {
